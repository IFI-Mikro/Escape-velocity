...
void loop() {
  ...
  if (memoryMode) { // wait on start buttonpress
    ...
    if (random(42) == 41L){
      for (int i = 0; i<12; i++) {
        setASwipe(i, wallRed, 0);
        setBSwipe(i, wallBlue, 0);
      }
      for (int i = 11; i>=0; i--) {
        setASwipe(i, wallBlue, 0);
        setBSwipe(i, wallRed, 0);
      }
      for(int i = 0; i<12; i++) {
        setBSwipe(i, wallOff, 0);
        setASwipe(i, wallOff, 0);
      }
    }
    // TODO
...
void generateSeq() {
  ...
  for (int i = 0; i < maxSeqLength; i++) {
    ...
    do {
      rnd = random(0, 42); // var orginalt satt til 41? 42 er vel mer riktig?
    } while (rnd == lastNr);
    seq[i] = rnd;
    lastNr = rnd;
  }
...
void randomSwiping(int num) {
  uint32_t a_color;
  for (int j = 0; j<num; j++) {
    a_color = (a_color == wallBlue)? wallRed : wallBlue;
    switch(int(random(8))){
      case 0: swipeAClear(a_color, 0); break; case 1: swipeBClear(a_color, 0); break; case 2: swipeCClear(a_color, 0); break; case 3: swipeDClear(a_color, 0); break;
      case 4: midSwipeAClear(a_color, 0); break; case 5: midSwipeBClear(a_color, 0); break; case 6: midSwipeCClear(a_color, 0); break; case 7: midSwipeDClear(a_color, 0); break;
    }
  }
}

int numberButtons() { // THIS IS A BIT FINICKY AND SHOULD BE REMADE..... 
    clearClock();
