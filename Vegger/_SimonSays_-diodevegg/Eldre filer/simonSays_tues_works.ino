...
void greenFlash(int nr, int nrFlashes) {
  for (int i = 0; i < nrFlashes; i++) {
    flashPixel(nr, 500, wallGreen);
    delay(500);
  }
}

//Flashes a group of LEDs
void flashGroup(int group[], int arrLength, int delay_time) {
  for (int i = 0; i < arrLength; i ++) {
    wallStrip.setPixelColor(led(group[i]), defColor);
  }
  wallStrip.show();
  delay(delay_time);
  for (int i = 0; i < arrLength; i ++) {
...
void rowFlash(int delayTime) {
  for (int i = 0; i < 7; i++) {
    flashGroup(row[i], 6, delayTime);
  }
}
void columnFlash(int delayTime) {
  for (int i = 0; i < 6; i++) {
    flashGroup(column[i], 7, delayTime);
  }
}
