/*
 * Eric Reber 15.4.2022
 * 
 * En liten justering av koden til Isak Bjørn fra 2018.
 * Eneste justering var et forsøk på å gjøre det litt mer tilfeldig.
 * Ønsker at man skal få en tilbakemelding når man nærmer seg en frekvens,
 * vet ikke om det fungerte så bra
 */
...
void equalizer(int freq) {
   ...
   */
  strip_clear();
  int randFreq[] = {rndFreq1, rndFreq2, rndFreq3, rndFreq4};
  for (int i = 0; i < 32; i++) {
    if (i == 31 || freq < floorFreq + i * freqStep ) {
      strip.setPixelColor(i, strip.Color(0, 255, 150));
      for (int j = 0; j < sizeof(randFreq); j++){
        if (freq < randFreq[j]+freqStep && freq > randFreq[j]-freqStep){
